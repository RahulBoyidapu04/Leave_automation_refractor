...
def test_convert_cl_to_al_param(leave_type, start, end, expected):
...
@pytest.fixture
def test_user(db):
    user = User(id=1, username="testuser", password_hash="hash", role="associate", team_id=1)
    db.add(user)
    db.commit()
    return user

@pytest.fixture
def setup_leave_balance(db, test_user):
...
@pytest.fixture
def setup_threshold(db, test_user):
...
def test_get_leave_balance(db, test_user, setup_leave_balance):
    balance = get_leave_balance(db, test_user.id, "AL")
    assert balance == 10

def test_decrement_leave_balance(db, test_user, setup_leave_balance):
    success = decrement_leave_balance(db, test_user.id, "AL", 3)
    assert success
    updated = get_leave_balance(db, test_user.id, "AL")
    assert updated == 7

def test_decrement_leave_balance_insufficient(db, test_user, setup_leave_balance):
...
def test_increment_monthly_leave(db, test_user, setup_threshold):
    increment_monthly_leave(db, test_user.id)
    updated = get_monthly_leave_count(db, test_user.id)
    assert updated == 5

# ---------- Test Leave Approval Logic ----------
def test_process_leave_approval(db, test_user, setup_leave_balance):
    data = {
...
def test_process_leave_rejection_due_to_balance(db, test_user):
...
def test_process_leave_rejection_due_to_threshold(db, test_user, setup_leave_balance, setup_threshold):
...
def test_process_optional_leave_rejection(db, test_user, setup_leave_balance):
    data = {
        "user_id": test_user.id,
        "leave_type": "Optional",
        "start_date": datetime.now().strftime("%Y-%m-%d"),
...
def test_next_30_day_shrinkage(db, test_user):
